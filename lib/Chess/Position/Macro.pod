...
Same as L<Chess::Position/castling>.

=item B<cp_pos_white_king_side_castling_right(POS)>

True if white can castle on the king-side in the L<Chess::Position> B<POS>.

L-value: no
=item B<cp_pos_white_queen_side_castling_right(POS)>

True if white can castle on the queen-side in the L<Chess::Position> B<POS>.

L-value: no
=item B<cp_pos_black_king_side_castling_right(POS)>

True if black can castle on the king-side in the L<Chess::Position> B<POS>.

L-value: no
=item B<cp_pos_black_queen_side_castling_right(POS)>

True if black can castle on the queen-side in the L<Chess::Position> B<POS>.
...
Get or set the bit-shift offset of the white king square of
L<Chess::Position> B<POS>.

L-value: yes.

=item B<cp_pos_b_king_shift(POS)>

Get or set the bit-shift offset of the black king square of
L<Chess::Position> B<POS>.
...
Get or set the in-check bitboard of the L<Chess::Position> B<POS>.
...
=item B<cp_bb_count_isolated_trailing_zbits(BITBOARD)>
...
Calculate the absolve value of B<INTEGER> without branching.
