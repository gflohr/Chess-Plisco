...
=item B<cp_pos_w_king_shift(POS)>

Same as L<Chess::Position/whiteKingShift>.

=item B<cp_pos_b_king_shift(POS)>

Same as L<Chess::Position/blackKingShift>.
...
=item B<cp_move_set_from(MOVE, PROMOTE)>
...
Clear all bits but the msot significant one in B<BITBOARD>.

Important! This works only under the "use integer" pragma!

=item B<cp_bitboard_clear_least_set(BITBOARD)>

Clear the least signficant set bit in B<BITBOARD>.
...
and B<BITBOARD> B<must> have a popcount of exactly 1, in other words, it must
