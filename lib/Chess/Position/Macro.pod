...
=item B<cp_pos_castling(POS)>

Get or set the king-side castling rights bitmask from the L<Chess::Position> B<POS>.

L-value: yes.

=item B<cp_pos_white_king_side_castling(POS)>

True if white can castle on the king-side in the L<Chess::Position> B<POS>.

L-value: no
=item B<cp_pos_white_queen_side_castling(POS)>

True if white can castle on the queen-side in the L<Chess::Position> B<POS>.

L-value: no
=item B<cp_pos_black_king_side_castling(POS)>

True if black can castle on the king-side in the L<Chess::Position> B<POS>.

L-value: no
=item B<cp_pos_black_queen_side_castling(POS)>
