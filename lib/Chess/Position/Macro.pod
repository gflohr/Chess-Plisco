...
=head1 SYNOPSIS
    ...
    my $white_pieces = cp_pos_white_pieces(Chess::Position->new);
...
or class methods of L<Chess::Position>.  You should only use the macros
...
=item Your code becomes less readable.
...
Remember that the macros ar expanded by a rather dumb search and replace.  That
...
Same as L<Chess::Position/whitePieces>.

=item B<cp_pos_black_pieces(POS)>

Same as L<Chess::Position/blackPieces>.

=item B<cp_pos_kings(POS)>

Same as L<Chess::Position/kings>.

=item B<cp_pos_queens(POS)>

Same as L<Chess::Position/queens>.

=item B<cp_pos_rooks(POS)>

Same as L<Chess::Position/rooks>.

=item B<cp_pos_bishops(POS)>

Same as L<Chess::Position/bishops>.

=item B<cp_pos_knights(POS)>

Same as L<Chess::Position/knights>.

=item B<cp_pos_pawns(POS)>

Same as L<Chess::Position/pawns>.

=item B<cp_pos_to_move(POS)>

Same as L<Chess::Position/toMove>.

=item B<cp_pos_castling_rights(POS)>

Same as L<Chess::Position/castlingRights>.

=item B<cp_pos_white_king_side_castling_right(POS)>

Same as L<Chess::Position/whiteKingSideCastlingRight>.

=item B<cp_pos_white_queen_side_castling_right(POS)>

Same as L<Chess::Position/whiteQueenSideCastlingRight>.

=item B<cp_pos_black_king_side_castling_right(POS)>

Same as L<Chess::Position/blackKingSideCastlingRight>.

=item B<cp_pos_black_queen_side_castling_right(POS)>

Same as L<Chess::Position/blackQueenSideCastlingRight>.

=item B<cp_pos_half_move_clock(POS)>

Same as L<Chess::Position/halfMoveClock>.

=item B<cp_pos_half_moves(POS)>

Same as L<Chess::Position/halfMoves>.

=item B<cp_pos_en_passant_shift(POS)>

Same as L<Chess::Position/enPassantShift>.

=item B<cp_pos_king_shift(POS)>

Same as L<Chess::Position/kingShift>.

=item B<cp_pos_in_check(POS)>

Same as L<Chess::Position/inCheck>.

=item B<cp_pos_evasion(POS)>

Same as L<Chess::Position/evasion>.

=item B<cp_pos_evasion_squares(POS)>

Same as L<Chess::Position/evasionSquares>.

=item B<cp_pos_signature(POS)>

Same as L<Chess::Position/signature>.

=item B<cp_pos_material(POS)>

Same as L<Chess::Position/material>.

=item B<cp_pos_info(POS)>

Same as L<Chess::Position/info>.  See also L</Position Info Macros> below!
...
Same as L</cp_pos_castling_ights> but work directly on position
...
Same as L<Chess::Position/moveTo>.

=item B<cp_move_set_to(MOVE, TO)>

Same as L<Chess::Position/moveSetTo> but modifies B<MOVE> directly.

=item B<cp_move_from(MOVE)>

Same as L<Chess::Position/moveFrom>.

=item B<cp_move_set_from(MOVE, FROM)>

Same as L<Chess::Position/moveSetFrom> but modifies B<MOVE> directly.

=item B<cp_move_promote(MOVE)>

Same as L<Chess::Position/movePromote>.

=item B<cp_move_set_promote(MOVE, PROMOTE)>

Same as L<Chess::Position/moveSetPromote> but modifies B<MOVE> directly.

=item B<cp_move_piece(MOVE)>

Same as L<Chess::Position/movePiece>.

=item B<cp_move_set_piece(MOVE, PIECE)>

Same as L<Chess::Position/moveSetPiece> but modifies B<MOVE> directly.

=item B<cp_move_captured(MOVE)>

Same as L<Chess::Position/moveCaptured>.

=item B<cp_move_set_captured(MOVE, PIECE)>

Same as L<Chess::Position/moveSetCaptured> but modifies B<MOVE> directly.
...
Same as L<Chess::Position/moveSetColor> but modifies B<MOVE> directly.

=item B<cp_move_equivalent(MOVE)>

Same as L<Chess::Position/moveEquivalent>.

=item B<cp_move_coordinate_notation(MOVE)>

Same as L<Chess::Position/moveCoordinateNotation>.
...
See also L<Chess::Position/bitboardPopcount>!

Important! This works only under the "use integer" pragma!

=item B<cp_bitboard_clear_but_least_set(BITBOARD)>

Same as L<Chess::Position/bitboardclearButLeastSet> but modifies
B<BITBOARD> directly.
...
Same as L<Chess::Position/bitboardclearButMostSet> but modifies
B<BITBOARD> directly.
...
Same as L<Chess::Position/clearLeastSet> but modifies B<BITBOARD> directly.

Important! This works only under the "use integer" pragma!

=item B<cp_bitboard_count_trailing_zbits(BITBOARD)>

Same as L<Chess::Position/countTrailingZbits>.

=item B<cp_bitboard_count_isolated_trailing_zbits(BITBOARD)>

Same as L<Chess::Position/countIsolatedTrailingZbits>.
