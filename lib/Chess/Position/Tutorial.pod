...
The methods C<SAN> and C<coordinateNotation> can be used to print a move in
humand-readable format:

    $pos->SAN($move);
    # Nxe7#
    $pos->coordinateNotation($move);
    # d6e7
...
faster:
    ...
    while ($white_pawn_mask) {
        ...
        my $shift = cp_bb_count_isolated_trailing_zbits $shift_mask;
...
macro `cp_bb_count_isolated_trailing_zbits()` you get the position of that bit in the
