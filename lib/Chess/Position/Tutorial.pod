...
branching (conditionals).  In the case of L<Chess::Position> it turned out
...
A move in C<Chess::Position> is simply an integer. The individual bits of a
move are:
...
The piece that moves, the piece is the piece that stands on the
...
In general, you will prefer print moves in Standard Algebraic Notation.
...
C<cp_bb_count_trailing_zbits()> you get the position of that bit in the
...
from L<Chess::Position::Macro> for it:
    ...
    $white_can_castle_king_side  = cp_pos_white_king_side_castle $pos;
    $white_can_castle_queen_side = cp_pos_white_queen_side_castle $pos;
    $black_can_castle_king_side  = cp_pos_black_king_side_castle $pos;
    $black_can_castle_queen_side = cp_pos_black_queen_side_castle $pos;
...
Example:
    ...
    # or $number_of_pieces = cp_bb_popcount $occupancy;
...
of L<Chess::Position> is an array reference, and the constants
...
The variable C<$bitboard> now contains the correct bitboard for the piece.
