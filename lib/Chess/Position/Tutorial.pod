...
=item * 20-.. (19-..): raw material balance (viewed from the side to move)

=item * 19-21 (18-20): piece that gets removed (resp. victim) if any
...
derived from the position that the move is applied to, and they are not
always present:
