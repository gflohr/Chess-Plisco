...
and a so-called evasion bitboard (64 bits).  To understand the evasion
...
Option 1 will always work. In this case, the evasion bitboard will be 0 (and
can be ignored).

Option 2 only works if there is just one piece giving check. In this case,
the evasion bitboard is identical to the bitboard of the pieces giving check.
...
square. In this case, the evasion bitboard includes all squares between the
...
This is the value that is relevant for claiming a draw according to the
50-move rule.
...
Return a 4-bit value. The highest bit signals whether en passant is possible.
The lower 3 bit give the en passant file.

There is no method B<enPassantFile> and that is by design.  If en passant is
possible on the a file, the method would return 0 which is the a file encoded.
...
=over 8
    ...
    if ($ep) {
        my $ep_file = $ep & 7;
        my $ep_square = $pos->enPassantFileToShift($ep);
    }
=back

However, there is no need to mask out the highest bit because the library
does that for you wherever an en passant file is expected.

=item B<enPassantShift>

If en-passant is possible, returns the shift of the square where the capturing
pawn has to move. If en-passant is not possible, 0 is returned. 0 happens to be
the shift of "a1" but "a1" is not a valid en-passant square. It is therefore
safe to use this method in boolean context.
...
Returns the move (as an integer) that has led to the position.  This is only
defined, after one of B<applyMove()>, B<doMove()>, or B<move()> has been called.
...
Sets the en passant flag in B<MOVE> to B<COLOR> and returns the move.
...
Engines should use the method L</unmove> instead because it is faster.
...
=item B<enPassantFileToShift(FILE, COLOUR)>

Given an en passant file B<FILE> return the shift of the en passant square
for colour B<C>. Instead of a file (0-8) you can also pass a complete
en passant info (0-16) where bit 4 is set.
...
=item B<POS-E<gt>[CP_POS_HALFMOVES]>
