...
=item B<move(MOVE[, NO_BACKUP]>)

Does the move B<MOVE> which must be legal because no legality check is
performed.

The method returns state information that can later be used to undo the move
and reset the position to the state it had before the move was applied. The
state information is in fact a copy of the object itself. If you pass a
truthy value for B<NO_BACKUP> this copy is not made which safes a little time.

Rationale: If you search a position you will often have code like this:
...
That has an avoidable overhead. First, you can avoid creating a backup
of the position over and over again. And second, you can save one method
invocation:
...
This saves just a little time but when you are searching hundreds of thousands
of positions, this is signifcant.
