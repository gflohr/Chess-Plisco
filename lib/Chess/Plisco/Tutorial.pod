...
=item * C<pawns> - one bitboard for the pawns (of both colours)

=item * C<knights> - one bitboard for the knights (of both colours)

=item * C<bishops> - one bitboard for the bishops (of both colours)

=item * C<rooks> - one bitboard for the rooks (of both colours)

=item * C<queens> - one bitboard for the queens (of both colours)

=item * C<kings> - one bitboard for the kings (of both colours)
...
You can think of it as FEN with meta data but without move numbers.
...
=item * 1-3: piece that moves (resp. piece)

=item * 4-6: piece that gets captured if any

=item * 7-9: promotion piece if any

=item * 10-15: from shift (0-63)

=item * 16-21: to shift (0-63)

=item * 22: side (0 for white, 1 for black) that moves

=item * 23: 1 for en-passant moves, 0 otherwise.

=back

Only bits 7 to 21 are really needed to characterize a move, and the bits
7 to 9 are only needed for pawn promotions. The other bits can be
...
=item * C<cp_move_colour($move)>: the colour that moves

=item * C<cp_move_color($move)>: the colour that moves

=item * C<cp_move_en_passant($move)>: the en-passant flag

=item * C<cp_move_significant($move)>: the significant bits of a move
...
L<Chess::Plisco> uses the brute force approach because it is faster.  But
...
The state information is - at the moment - just a shallow copy of the position
object and invoking undoMove is another way of saying:

    @$pos = @$state

Note that C<$pos> is still blessed after this operation.
...
The above example could be re-written like this:
    ...
    foreach $move (@moves) {
        $pos->checkPseudoLegalMove($move) or next;
        print join '|', $pos->SAN($move), $pos->moveCoordinateNotation($move);
    }
...
not rely on the current implementation. At the moment, it includes all moves
that are possible by the movement patterns of the pieces but leave the
king in check.
...
=head3 Toggling Colour

C<CP_WHITE> is defined as 0 and C<CP_BLACK> is defined as 1.  Therefore,
toggling a colour is as easy as:

    $colour = !$colour; # because CP_WHITE == !CP_BLACK
...
by a pawn of the same colour.  It can reach d5 B<and> c6 because c6 is
...
result you have to mask out the pieces of the same colour:
