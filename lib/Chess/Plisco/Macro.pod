...
=item B<cp_pos_en_passant(POS)>

Same as L<Chess::Plisco/enPassant>.

=item B<cp_pos_en_passant_file(POS)>

Same as L<Chess::Plisco/enPassantFile>.
...
=item B<cp_pos_info_en_passant(INFO)>

Same as L</cp_pos_en_passant> but work directly on position
info B<INFO> as returned by L</cp_pos_info>.

=item B<cp_pos_info_en_passant_file(INFO)>

Same as L</cp_pos_en_passant_file> but work directly on position
info B<INFO> as returned by L</cp_pos_info>.
