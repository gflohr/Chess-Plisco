...
A square is one square of the chess board as a string like "e4" or "f7".
...
Return the number of reversible moves immediately leading to the current
...
=item the first king move of each side

Reason: The side to move loses the castling rights.

=item the first move of each rook of each side
...
The half-move-clock because both players can claim a draw, when the half-move
clock has reached 50.

=item B<inCheck>

Returns false if the side to move is not in check, a truthy value otherwise.
The truthy value returns is also a bitboard of all pieces giving check.
...
=item 0x1 if white can still castle king-side

=item 0x2 if white can still castle queen-side

=item 0x4 if black can still castle king-side

=item 0x8 if black can still castle queen-side
...
The method returns state that can later be used to undo the move and reset
the position to the state it had before the move was applied.
...
It does the same as L<Chess::Position::Macro/cp_square_to_shift>.
...
It does the same as L<Chess::Position::Macro/cp_square_to_coordinates>.
...
It does the same as L<Chess::Position::Macro/cp_shift_to_square>.
...
It does the same as L<Chess::Position::Macro/cp_coordinates_to_square>.
...
It does the same as L<Chess::Position::Macro/cp_coordinates_to_shift>.
