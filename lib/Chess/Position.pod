...
Like </pseudoLegalMoves> but only returns "interesting" moves.  Interesting
...
one release of B<Chess::Position>.  In order to compute different signatures,
...
=item B<setMovePromote(MOVE, PROMOTE)>
...
=item B<setMovePiece(MOVE, PIECE)>
...
victim of a capture and is a valid target square.

See L<Chess::Position::Tutorial/Understanding C<rMagic> and C<bMagic>> for
more information.
...
Returns a pseudo-random integer created by the well-knows xor-shift
...
=head3 B<POS-E<gt>[CP_POS_WHITE_PIECES]>
...
These are bitboard of all files ("a" to "h") of the chess board.

=head3 CP_1_MASK .. CP_8_MASK

These are bitboard of all ranks ("1" to "8") of the chess board.
